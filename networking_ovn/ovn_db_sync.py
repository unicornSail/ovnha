...
class OvnNbSynchronizer(OvnDbSynchronizer):
    ...
    def sync_routers_and_rports(self, ctx):
        ...
        fips = self.l3_plugin.get_floatingips(ctx, {'router_id': db_routers_keys})
                ...
                for gwport, gwport_nets in lrouter['gateway_ports'].items():
                    if gwport in db_gateway_ports:
                        ...
                        del_gwrouter_ports_list.append(
                            {'port': gwport, 'lrouter': lrouter['name'],
                             'next_hop': lrouter['next_hop']})
                ...
                try:
                    ...
                    if 'floating_ips' in router:
                        update_fips_list.append(
                            {'id': router['id'], 'add': router['floating_ips'],
                             'del': []})
                except RuntimeError:
        ...
        for gwport_info in del_gwrouter_ports_list:
            LOG.warning(_LW("Gateway Port found in OVN but not in "
                            "Neutron, port_id=%s"), gwport_info['port'])
            if self.mode == SYNC_MODE_REPAIR:
                LOG.warning(_LW("Deleting the port %s from OVN NB DB"),
                            gwport_info['port'])
                self.l3_plugin.delete_gw_router_port_in_ovn(gwport_info['lrouter'],
                                                        gwport_info['port'],
                                                        gwport_info['next_hop'])

        for gwp_id, gwport in db_gateway_ports.items():
            LOG.warning(_LW("Gateway Port found in Neutron but not in OVN "
                            "DB, gateway port_id=%s"), gwport['id'])
            if self.mode == SYNC_MODE_REPAIR:
                try:
                    LOG.warning(_LW("Creating the gateway port %s in "
                                    "OVN NB DB"), gwport['id'])
                    self.l3_plugin.create_gw_router_port_in_ovn(ctx, gwport['device_id'],
                                                                gwport)
                except RuntimeError:
                    LOG.warning(_LW("Create gateway port in OVN "
                                    "NB failed for"
        ...
        with self.ovn_api.transaction(check_error=True) as txn:
            ...
            for fip in update_fips_list:
                if fip['del']:
                    ...
                    if self.mode == SYNC_MODE_REPAIR:
                        LOG.warning(_LW("Delete floating ips %s from OVN NB DB"),
                                    fip['del'])
                        for ifip in fip['del']:
                            txn.add(self.ovn_api.delete_nat(utils.ovn_gateway_name(fip['id']),
                ...
                if fip['add']:
                    ...
                    if self.mode == SYNC_MODE_REPAIR:
                        LOG.warning(_LW("Add floating ips %s to OVN NB DB"),
                                    fip['add'])
                        for ifip in fip['add']:
                            txn.add(self.ovn_api.add_nat(utils.ovn_gateway_name(fip['id']),
                                    logical_ip=ifip['fixed_ip_address'],
                                    external_ip=ifip['floating_ip_address'],
                                    type='dnat_and_snat'))
